...
# Define the N-ary tree node for implementing Pot-Holes
class NaryTreeNode:
...
# Define the Trie structure for autofill suggestions and narrowing down the search
class Trie:
    def __init__(self):
        self.root = TrieNode()
...
# Shortest path alogrithm Used for finding the shortest Path using Geocode Coordinates
class Dijkstra:
    ...
    def shortest_path(self, start_node, end_node):
        ...
        shortest_paths = {node: float('infinity') for node in self.graph.nodes}
        ...
        while priority_queue:
            ...
            # Iterate over neighbors and retrieve weights
            for neighbor in self.graph.neighbors(current_node):
                # Retrieve the edge data dictionary and get the 'length' (weight)
                weight = self.graph.get_edge_data(current_node, neighbor).get('length', 1)
        ...
        return self.reconstruct_path(previous_nodes, start_node, end_node)

    def reconstruct_path(self, previous_nodes, start_node, end_node):
        path = []
...
class MapApp:
    ...
    def populate_trie(self):
        # Populate the Trie with some city names
    ...
    def get_directions(self):
        ...
        try:
            ...
            # Create an instance of the custom Dijkstra's algorithm class
            dijkstra = Dijkstra(G)

            # Get the shortest path using the custom Dijkstra's algorithm
            route = dijkstra.shortest_path(start_node, end_node)
